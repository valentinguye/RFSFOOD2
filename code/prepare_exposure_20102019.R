...
dir.create(here("temp_data","exposures_20102019"))
...
POSTtreatment_years <- c(2010:2019)
year <- 2010
# list to store annual data sets prepared
wide_fb_list <- list()

for(year in POSTtreatment_years){
  fb <- read.csv(here("input_data", "exposure_variables", paste0("FAOSTAT-foodbalances_allcountries_aggritems_",year,".csv")))
  ...
  fb <- dplyr::select(fb, -Area.Code..FAO.)
  ...
  fb[duplicated(fb[,c("country", "Element", "Item")]), ] %>% nrow() 
  ...
  # Some items have different names than in old FAO data set, and we need to make them match 
  fb$Item[!(fb$Item %in% fb$Item)] %>% unique()
  fb$Item[fb$Item=="Rice and products"] <- "Rice (Milled Equivalent)"
  fb$Item[fb$Item=="Groundnuts"] <- "Groundnuts (Shelled Eq)"
  ...
  # SPECIFIC RENAMING TO THESE FAOSTAT WAVES
  # In these waves of FAOSTAT data, there is only one Sudan, called "Sudan", which need to be renamed to match "Sudan (former)" in old data 
  wide_fb$country[wide_fb$country=="Sudan"] <- "Sudan (former)"
  
  # Keep track of the year 
  wide_fb$year <- year
  
  wide_fb_list[[match(year, POSTtreatment_years)]] <- wide_fb
...
### AVERAGE OVER YEARS 
POSTtreat_year_sets <- list(`2010_2019` = c(2010:2019))

csfb <- list()
for(POSTtreat_period in POSTtreat_year_sets){
  
  csfb <- ddply(pfb[pfb$year %in% POSTtreat_period, ], 
                "country", summarise, 
  ...
  saveRDS(csfb, file = here("temp_data", "exposures_20102019", paste0("dependency_",
                                                             min(POSTtreat_period),
                                                             max(POSTtreat_period),".Rdata")))
